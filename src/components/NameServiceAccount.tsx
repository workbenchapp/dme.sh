...
function NameServiceAccountView(props: { account: ProgramAccount }) {
  ...
  const solTLD = new web3.PublicKey(
  ...
  React.useEffect(() => {
    ...
    bonFidaNameService
      .performReverseLookup(connection, account.pubkey)
      .then((nameResult) => {
        setName(nameResult);
    ...
    bonFidaNameService.NameRegistryState.retrieve(
      connection,
      account.pubkey
    ).then((nrs) => {
      ...
      if (nrs.registry.parentName) {
        ...
        bonFidaNameService
          .performReverseLookup(connection, nrs.registry.parentName)
          .then((nameResult) => {
