...
const NamesPage = () => {
    ...
    const [waitingForRequest, setWaitingForRequest] = React.useState<boolean>(false);

    const userAccount = new web3.PublicKey('Ld8u5TPGyduTpFoTAn6WGDis2yAGAd3hXqi2gUeiu4d');


    return (<Container><Heading>Name service accounts          <Button disabled={buttonUsed} onClick={() => {
        ...
        connection.getParsedProgramAccounts(splNameService.NAME_PROGRAM_ID, { filters })
        ...
        <CardGroup>
            {accounts.slice(0, 4).map((account: ProgramAccount) => {
                return (<NameServiceAccountView key={account.pubkey.toString()} account={account}></NameServiceAccountView>);
            })}
